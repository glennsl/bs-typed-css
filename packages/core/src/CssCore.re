...
type singleTransition('a) = (
  value([< singleTransitionProperty] as 'a),
  time,
  timingFunction,
  time
);
...
module type Values = {
  ...
  */
  ...
  let translate:
    (
      value([< | `length | `zwro | `percentage]),
      value([< | `length | `zero | `percentage])
    ) =>
    transform;
...
module type Properties = {
  ...
  let margin2:
    (~v: value([< marginWidth]), ~h: value([< marginWidth])) => declaration;
  let margin3:
    (
      ~top: value([< marginWidth]),
      ~h: value([< marginWidth]),
  ...
  let padding2:
    (~v: value([< paddingWidth]), ~h: value([< paddingWidth])) => declaration;
  let padding3:
    (
      ~top: value([< paddingWidth]),
  ...
  let border2:
    ...
    declaration;
  let border3:
    (value([< lineWidth]), value([< lineStyle]), value([< | `color])) =>
    declaration;
  ...
  let borderRadius:
    value([< | `length | `zero | `percentage | `universal]) => declaration;
  /* TODO?: borderRadius2-4 */
  let borderTopLeftRadius:
    value([< | `length | `zero | `percentage | `universal]) => declaration;
  let borderTopLeftRadius2:
    (
      ~v: value([< | `length | `zero | `percentage | `universal]),
      ~h: value([< | `length | `zero | `percentage | `universal])
    ) =>
    declaration;
  let borderTopRightRadius:
    value([< | `length | `zero | `percentage | `universal]) => declaration;
  let borderTopRightRadius2:
    (
      ~v: value([< | `length | `zero | `percentage | `universal]),
      ~h: value([< | `length | `zero | `percentage | `universal])
    ) =>
  ...
  let display:
    ...
    declaration;
  let position:
    value([< | `static | `relative | `absolute | `fixed | `universal]) =>
    declaration;
  let offsetTop:
    value([< | `length | `zero | `percentage | `auto | `universal]) =>
    declaration;
  let offsetRight:
    value([< | `length | `zero | `percentage | `auto | `universal]) =>
    declaration;
  ...
  let clear:
    value([< | `none | `left | `right | `both | `universal]) => declaration;
  let zIndex: value([< | `auto | `integer | `universal]) => declaration;
  let direction: value([< | `ltr | `rtl | `universal]) => declaration;
  let unicodeBidi:
    value([< | `normal | `embed | `bidiOverride | `universal]) => declaration;
  let width:
    value([< | `length | `zero | `percentage | `auto | `universal]) =>
    declaration;
  let minWidth:
    value([< | `length | `zero | `percentage | `universal]) => declaration;
  let maxWidth:
    value([< | `length | `zero | `percentage | `none | `universal]) =>
    declaration;
  let height:
    value([< | `length | `zero | `percentage | `auto | `universal]) =>
    declaration;
  ...
  let lineHeight:
    value(
      [< | `normal | `number | `length | `zero | `percentage | `universal]
    ) =>
    declaration; /* TODO? move to fonts */
  let verticalAlign:
    value(
      [<
        | `baseline
   ...
   */
  let overflow:
    value([< | `visible | `hidden | `scroll | `auto | `universal]) =>
    declaration;
  let clip: value([< | `shape | `auto | `universal]) => declaration;
  let visibility:
    value([< | `visible | `hidden | `collapse | `universal]) => declaration;
  ...
  let background:
    value([< | `color | `uri | `none | `universal]) => declaration;
  let backgroundColor: value([< | `color | `universal]) => declaration;
  let backgroundImage: value([< | `uri | `none | `universal]) => declaration;
  let backgroundRepeat:
    value([< | `repeat | `repeatX | `repeatY | `noRepeat | `universal]) =>
    declaration;
  let backgroundAttachment:
    value([< | `scroll | `fixed | `universal]) => declaration;
  let backgroundPosition: value([< | `universal]) => declaration;
  let backgroundPosition2:
    (
      ~h: value([< | `percentage | `length | `zero | `left | `center | `right]),
      ~v: value(
            [< | `percentage | `length | `zero | `top | `center | `bottom]
          )
    ) =>
    declaration;
  ...
  let shadow:
    ...
    value([ | `shadow]); /* TODO: move to values */
  ...
  let fontStyle:
    value([< | `normal | `italic | `oblique | `universal]) => declaration;
  let fontVariant:
    value([< | `normal | `smallCaps | `universal]) => declaration;
  let fontWeight:
    value(
      [<
        | `normal
  ...
  let textIndent:
    value([< | `length | `zero | `percentage | `universal]) => declaration;
  let textAlign:
    value([< | `left | `right | `center | `justify | `universal]) =>
    declaration;
  let textDecoration:
    value(
      [<
    ...
    declaration;
  let letterSpacing:
    value([< | `normal | `length | `zero | `universal]) => declaration;
  let wordSpacing:
    value([< | `normal | `length | `zero | `universal]) => declaration;
  let textTransform:
    value([< | `capitalize | `uppercase | `lowercase | `none | `universal]) =>
    declaration;
  let whiteSpace:
    value(
      [< | `normal | `pre | `nowrap | `preWrap | `preLine | `universal]
    ) =>
    declaration;
  ...
  let transitionProperty:
    value([< | `none | singleTransitionProperty | `universal]) => declaration;
  let transitionProperties:
    list(value([< singleTransitionProperty])) => declaration;
  ...
  let transitionTimingFunction:
    value([< | `timingFunction | `universal]) => declaration;
  ...
  let transformOrigin2:
    ...
    declaration;
  let transformBox:
    value([< | `borderBox | `fillBox | `viewBox | `universal]) => declaration;

  /**
   * Flexible Box Layout Model
   */
  let flexDirection:
    value([< | `row | `rowReverse | `column | `columnReverse | `universal]) =>
    declaration;
  let flexWrap:
    value([< | `nowrap | `wrap | `wrapReverse | `universal]) => declaration;
  let flexFlow:
    value(
      [<
        | `row
  ...
  let flexFlow2:
    (
      value([< | `row | `rowReverse | `column | `columnReverse]),
      value([< | `nowrap | `wrap | `wrapReverse])
    ) =>
    declaration;
  let order: int => declaration;
  let flex_:
    value([< | `integer | `content | `auto | `none | `universal]) =>
    declaration;
  let flex3:
    (
      ~grow: float,
      ~shrink: float,
      value([< | `content | `length | `zero | `percentage | `auto])
    ) =>
    declaration;
  let flexGrow: float => declaration;
  let flexShrink: float => declaration;
  let flexBasis:
    value([< | `content | `length | `percentage | `auto | `universal]) =>
    declaration;
  let justifyContent:
    value(
      [<
